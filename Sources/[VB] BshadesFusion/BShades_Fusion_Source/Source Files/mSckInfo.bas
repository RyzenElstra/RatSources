...
Public sckinfo_SocketHandle                 As Long
Public sckinfo_IsConnected                  As Boolean
Public sckinfo_IsBusy                       As Boolean
...
Public sckinfo_Tag As String

Public Sub WsInitialize(ObjectHost As Object) '.. Load the socket object. Must be called before doing anything.
...
Public Sub WsConnect(ByVal Host As String, ByVal Port As Long) '.. Connect to host:port
    ...
    sckinfo_SocketHandle = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)
    
    If sckinfo_SocketHandle < 0 Then Exit Sub

    If Connect(sckinfo_SocketHandle, sockdata, 16) <> 0 Then
        If sckinfo_SocketHandle Then WsClose
        Exit Sub
    End If
    
    If WSAAsyncSelect(sckinfo_SocketHandle, hwnd, ByVal WINSOCK_MESSAGE, ByVal FD_READ Or FD_WRITE Or FD_CONNECT Or FD_CLOSE) Then
        WsClose
    Else
        Call m_Object.sckInfo_Connect
        sckinfo_IsConnected = True
    End If
...
Public Sub WsClose() '.. Close socket but do not terminate object.
    ...
    sckinfo_IsConnected = False
    closesocket sckinfo_SocketHandle
End Sub
...
Private Function WindowProc(ByVal hwnd As Long, ByVal uMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
    ...
    If uMsg = WINSOCK_MESSAGE Then
        If wParam <> sckinfo_SocketHandle Then
            'Debug.Print "WindowProc: Invalid wParam vs sckinfo_SocketHandle argument. How the hell did this happen?"
            WsTerminate
        End If
        ...
        Select Case lParam
            ...
            Case FD_READ
                ...
                If LenB(sTemp) > 0 Then Call m_Object.sckInfo_DataArrival(sTemp, Len(sTemp))
            Case Else 'FD_CLOSE
                Call m_Object.sckInfo_CloseSck
                Call WsClose
        End Select
    Else
...
Private Sub SendBufferedData() '.. Send data buffered (so we don't block)
    Dim arrData() As Byte
    Dim lRet As Long, lTotal As Long, lBuffer As Long
    ...
    Do Until lRet = SOCKET_ERROR Or Len(m_strSendBuffer) = 0
        ...
        If lngBufferLength > SENDBUFFER_LENGTH Then
            ...
            arrData() = StrConv(Left$(m_strSendBuffer, SENDBUFFER_LENGTH), vbFromUnicode)
        Else
            arrData() = StrConv(m_strSendBuffer, vbFromUnicode)
        End If
        
        lRet = send(sckinfo_SocketHandle, arrData(0), lngBufferLength, 0&)
