...
begin
  ...
  try
    ...
    begin
      DeviceClassName := copy(Lista, 1, pos(SeparadorDevices, Lista) - 1);
      delete(Lista, 1, pos(SeparadorDevices, Lista) + length(SeparadorDevices) - 1);

      tvRoot := tvDevices.Items.Add(nil, DeviceClassName);

      TempStr := copy(Lista, 1, pos('##' + SeparadorDevices, Lista) - 1);
      delete(Lista, 1, pos('##' + SeparadorDevices, Lista) + 1);
      ...
      tvRoot.StateIndex := strtoint(copy(Lista, 1, pos(SeparadorDevices, Lista) - 1));
      delete(Lista, 1, pos(SeparadorDevices, Lista) + length(SeparadorDevices) - 1);
      ...
      begin
        ...
        DeviceName := copy(Lista, 1, pos(SeparadorDevices, Lista) - 1);
        delete(Lista, 1, pos(SeparadorDevices, Lista) + length(SeparadorDevices) - 1);
        ...
        dwIndex := strtoint(copy(Lista, 1, pos(SeparadorDevices, Lista) - 1));
        delete(Lista, 1, pos(SeparadorDevices, Lista) + length(SeparadorDevices) - 1);
        delete(Lista, 1, 2); // #13#10
...
begin
  ...
  try
    ...
    begin
      ...
      Item.Caption := copy(TempStr, 1, pos(SeparadorDevices, Tempstr) - 1);
      delete(TempStr, 1, pos(SeparadorDevices, Tempstr) + length(SeparadorDevices) - 1);
      Item.SubItems.Add(copy(TempStr, 1, pos(SeparadorDevices, Tempstr) - 1));
      delete(TempStr, 1, pos(SeparadorDevices, Tempstr) + length(SeparadorDevices) - 1);
