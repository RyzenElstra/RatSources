...
begin
  ...
  while (Permitidos <> '') do
  begin
    if Pos(';', Permitidos) > 0 then
    begin
      TempStr := Copy(Permitidos, 1, Pos(';', Permitidos) - 1);
      Delete(Permitidos, 1, Pos(';', Permitidos));
    end else TempStr := Permitidos;

    Result := pos(UpperString(TempStr), UpperString(TempJanela)) > 0;
    ...
    case wVirtKey of
{
  ...
  if (Length(result) > 0) and
     ...
     (pos('[', Result) > 0) and    // aceita #13#10, ' ', e outros se existirem
     (pos('Numpad', Result) <= 0)  // aceita numpad no resultado
     then
  begin
    DelBack := True;
