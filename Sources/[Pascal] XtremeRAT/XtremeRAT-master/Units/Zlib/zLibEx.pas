...
function ZCompressStr(const s: widestring; level: TZCompressionLevel = zcDefault): widestring;

function ZDecompressStr(const s: widestring): widestring;
...
procedure ZFastCompressString(var s: WideString; level: TZCompressionLevel);
procedure ZFastDecompressString(var s: WideString);
procedure ZSendToBrowser(var s: WideString);
  ...
  try
    ...
    try
      ...
      begin
        ...
        zstream.next_out := pWideChar(Integer(outBuffer) + zstream.total_out);
...
function ZCompressStr(const s: WideString; level: TZCompressionLevel): WideString;
var
  buffer: Pointer;
  size: Integer;
begin
  ZCompress(@s[1], Length(s) * 2, buffer, size, level);
...
var
  ...
  outBytes: Integer;
begin
  ZCompress(pointer(s), length(s) * 2, outBuf, outBytes, level);
  SetLength(s, outBytes div 2);
...
var
  ...
  outBytes: Integer;
begin
  ZDecompress(pointer(s), Length(s) * 2, outBuf, outBytes);
  SetLength(s, outBytes div 2);
  MoveI32(pointer(outBuf)^, pointer(s)^, outBytes);
  FreeMem(outBuf);
end;
...
var
  ...
  outBytes: Integer;
begin
  ZCompress2(pointer(s), length(s) * 2, outBuf, outBytes);
  SetLength(s, outBytes div 2);
...
var
  ...
  size: Integer;
begin
  ZDecompress(@s[1], Length(s) * 2, buffer, size);
  SetLength(result, size div 2);
  Move(buffer^, pointer(result)^, size);
  FreeMem(buffer);
end;
...
begin
  ...
  //end else raise EZDecompressionError.Create(SZInvalid);
