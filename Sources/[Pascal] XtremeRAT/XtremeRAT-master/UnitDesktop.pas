...
begin
  ...
  begin
    ...
    OriginalX := StrToInt(Copy(Recebido, 1, pos('|', recebido) - 1));
    delete(Recebido, 1, pos('|', recebido));
    OriginalY := StrToInt(Copy(Recebido, 1, pos('|', recebido) - 1));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = DESKTOPSTREAM then
  begin
    ServidorTransferencia := ConAux;
    if ServidorTransferencia.MasterIdentification <> 1234567890 then Exit;
