...
begin
  ...
  if Copy(Recebido, 1, pos('|', recebido) - 1) = MSNINFO then
  begin
    delete(recebido, 1, pos('|', recebido));
    if AdvListView1.Items.Count > 0 then AdvListview1.Items.Clear;
    ...
    Item.SubItems.Add(Copy(recebido, 1, pos(delimitadorComandos, recebido) - 1));
  ...
  begin
    ...
    begin
      ...
      SName := Copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
      delete(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
      delete(TempStr, 1, length(delimitadorComandos));

      FName := Copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
      delete(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
      delete(TempStr, 1, length(delimitadorComandos));

      Status := Copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
      delete(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
      delete(TempStr, 1, length(delimitadorComandos));

      Blocked := Copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
