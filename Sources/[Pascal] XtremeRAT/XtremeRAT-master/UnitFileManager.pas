...
begin
  ...
  begin
    if pos(traduzidos[280] + '\', Result) > 0 then delete(result, 1, pos('\', Result)) else
    if result = traduzidos[280] then result := '';
  end;

  if traduzidos[308] <> '' then
  begin
    if pos(traduzidos[308] + '\', Result) > 0 then delete(result, 1, pos('\', Result)) else
    if result = traduzidos[308] then result := '';
  end;

  if traduzidos[309] <> '' then
  begin
    if pos(traduzidos[309] + '\', Result) > 0 then delete(result, 1, pos('\', Result)) else
...
begin
  ...
  begin
    ...
    begin
      try
        Date1 := StrToDateTime(Copy(Item1.SubItems[3], 1, pos(' ', Item1.SubItems[3]) - 1));
        except
      end;
      try
        Date2 := StrToDateTime(Copy(Item2.SubItems[3], 1, pos(' ', Item2.SubItems[3]) - 1));
        except
      end;
...
begin
{
  ...
  while TempStr <> '' do
  begin
    TempDir := copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
...
begin
  ...
  begin
    ...
    if Pos('H', Item.SubItems[1]) > 0 then Item.Cut := True;

    Tn := Treeview1.Items.AddChild(Node, FindData.cFileName);
    if pos('H', GetAttrib(FindData)) > 0 then tn.Cut := True;
...
begin
  ...
  try
    ...
    if pos('H', ListView1.Selected.SubItems.Strings[1]) > 0 then
      Form.CheckBox1.Checked := true else
      Form.CheckBox1.Checked := false;
    if pos('S', ListView1.Selected.SubItems.Strings[1]) > 0 then
      Form.CheckBox2.Checked := true else
      Form.CheckBox2.Checked := false;
    if pos('R', ListView1.Selected.SubItems.Strings[1]) > 0 then
...
begin
  ...
  try
    ...
    if pos('H', ListView2.Selected.SubItems.Strings[1]) > 0 then
      Form.CheckBox1.Checked := true else
      Form.CheckBox1.Checked := false;
    if pos('S', ListView2.Selected.SubItems.Strings[1]) > 0 then
      Form.CheckBox2.Checked := true else
      Form.CheckBox2.Checked := false;
    if pos('R', ListView2.Selected.SubItems.Strings[1]) > 0 then
      Form.CheckBox3.Checked := true else
      Form.CheckBox3.Checked := false;
...
begin
  ...
  if Pos('\', ComboBox1.Text) > 2 then IniciarListagem(ComboBox1.Text);
...
begin
  ...
  while pos('\\', LastUploadFolder) > 0 do delete(LastUploadFolder, pos('\\', LastUploadFolder), 1);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Transfer.Transfer_RemoteFileName := Copy(TempStr, 1, pos('|', TempStr) - 1);
            delete(TempStr, 1, pos('|', TempStr));

            Transfer.Transfer_RemoteFileSize := StrToInt(Copy(TempStr, 1, pos('|', TempStr) - 1));
            delete(TempStr, 1, pos('|', TempStr));
            Transfer.Transfer_RemoteFileSize_string := FileSizeToStr(Transfer.Transfer_RemoteFileSize);

            Transfer.Transfer_LocalFileName := Copy(TempStr, 1, pos('|', TempStr) - 1);
            delete(TempStr, 1, pos('|', TempStr));

            Transfer.Transfer_LocalFileSize := StrToInt(Copy(TempStr, 1, pos('|', TempStr) - 1));
            delete(TempStr, 1, pos('|', TempStr));
...
begin
  ...
  RemoteFileName := Copy(ListaArquivosPasta, 1, pos(delimitadorcomandos, ListaArquivosPasta) - 1);
  delete(ListaArquivosPasta, 1, pos(delimitadorcomandos, ListaArquivosPasta) - 1);
...
begin
  ...
  while TempStr <> '' do
  begin
    RemoteFileName := Copy(TempStr, 1, pos(delimitadorcomandos, TempStr) - 1);
    delete(TempStr, 1, pos(delimitadorcomandos, TempStr) - 1);
    delete(TempStr, 1, length(delimitadorcomandos));
...
begin
  ...
  RemoteFileName := Copy(OneByOneList, 1, pos(delimitadorcomandos, OneByOneList) - 1);
  delete(OneByOneList, 1, pos(delimitadorcomandos, OneByOneList) - 1);
...
begin
  ...
  begin
    ...
    if Pos('H', ListView.Items[i].SubItems[1]) > 0 then ListView.Items[i].Cut := True;
  end;
end;
...
begin
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMSENDFTPYES then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    statusbar1.Panels.Items[0].Text := traduzidos[340] + ': "' + Recebido + '"';
  end else

  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMSENDFTPNO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMUPLOADERROR then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMDOWNLOADERROR then
  begin
    tempBool := False;
    delete(Recebido, 1, pos('|', Recebido));
    ...
    begin
      TempStr := Copy(OneByOneList, 1, pos(DelimitadorComandos, OneByOneList) - 1);
      Delete(OneByOneList, 1, pos(DelimitadorComandos, OneByOneList) - 1);
      Delete(OneByOneList, 1, Length(DelimitadorComandos));
    ...
    begin
      TempStr := Copy(ListaArquivosPasta, 1, pos(DelimitadorComandos, ListaArquivosPasta) - 1);
      Delete(ListaArquivosPasta, 1, pos(DelimitadorComandos, ListaArquivosPasta) - 1);
      Delete(ListaArquivosPasta, 1, length(DelimitadorComandos));
  ...
  begin
    ...
    TempInt := StrToInt(Copy(Recebido, 1, pos('|', Recebido) - 1)); //Tamanho do arquivo
    delete(Recebido, 1, pos('|', Recebido));
  ...
  begin
    ...
    while recebido <> '' do
    begin
      TempStr := Copy(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);  // FileName
      delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
      delete(Recebido, 1, length(delimitadorComandos));

      TempStr1 := Copy(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);  // FileImage
      delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
      delete(Recebido, 1, length(delimitadorComandos));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMSHAREDDRIVELIST then
  begin
    if ConAux.MasterIdentification = 1234567890 then
    try
      ConAux.Connection.Disconnect;
    ...
    begin
      ...
      delete(TempStr, 1, pos('|', Tempstr));
      if copy(TempStr, 1, pos('|', TempStr) - 1) = '8' then
      begin
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMSPECIALFOLDERS then
  begin
    delete(recebido, 1, pos('|', recebido));
    while recebido <> '' do
    begin
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMSPECIALFOLDERS2 then
  begin
    delete(recebido, 1, pos('|', recebido));
  ...
  begin
    ...
    while Recebido <> '' do
    begin
      TempStr := Copy(Recebido, 1, pos(#13#10, Recebido) - 1);
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMFILELIST then
  begin
    while FMFILELIST_STARTED = true do Application.ProcessMessages;
    FMFILELIST_STARTED := true;

    delete(recebido, 1, pos('|', recebido));
    TempNode := (SelectedNode[StrToInt(Copy(Recebido, 1, pos('|', recebido) - 1))]);
    delete(recebido, 1, pos('|', recebido));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = FMFILELIST2 then
  begin
    while FMFILELIST_STARTED = true do Application.ProcessMessages;
    FMFILELIST_STARTED := true;
