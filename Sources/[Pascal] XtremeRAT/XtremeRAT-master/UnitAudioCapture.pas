...
begin
  ...
  while pos(DelimitadorComandos, TempStr) > 0 do
  begin
    Item := AdvListView1.Items.Add;
    Item.ImageIndex := 75;

    s := Copy(TempStr, 1, pos('|', TempStr) - 1);
    ...
    delete(TempStr, 1, pos('|', TempStr));
    G := TGravacoes.Create;
    G.Objeto := Copy(TempStr, 1, pos(DelimitadorComandos, TempStr) - 1);
...
begin
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = AUDIOSTREAM then
  begin
