...
begin
  ...
  begin
    while SL <> '' do
    begin
      s := copy(sl, 1, pos(RegeditDelimitador, sl) - 1);
      delete(sl, 1, pos(RegeditDelimitador, sl) - 1);
      delete(sl, 1, length(RegeditDelimitador));

      delete(s, 1, pos(DelimitadorComandos, s) - 1);
      delete(s, 1, length(DelimitadorComandos));
      delete(s, 1, pos(DelimitadorComandos, s) - 1);
      delete(s, 1, length(DelimitadorComandos));

      s := Copy(s, 1, pos(DelimitadorComandos, s) - 3);

      while s <> '' do
      begin
        if pos(#13, s) > 0 then
        begin
          TempStr := TempStr + copy(s, 1, pos(#13, s) - 1) + #13#10;
          delete(s, 1, pos(#13, s));
        end else
        begin
    ...
    while pos(#13#10, sl) > 0 do
    begin
      TempStr := Copy(sl, 1, pos(#13#10, sl) - 1);
      delete(sl, 1, pos(#13#10, sl) + 1);
