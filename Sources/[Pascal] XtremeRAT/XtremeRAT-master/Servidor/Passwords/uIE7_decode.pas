...
function DecryptFromRegistry(ToCheck : tastring; Path : pansichar; Delimitador: string): string;
var
  History : tastring;
  i : Integer;
  m : Integer;
...
begin
  ...
  begin
    ...
    result := result + Copy(plain, 0, Pos(':',plain) - 1)  + Delimitador;
    result := result + Copy(plain,Pos(':',plain) + 1,Length(plain) - Pos(':',plain)) + Delimitador + #13#10;
  end;
end;
