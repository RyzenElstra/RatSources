...
begin
  ...
  while TempStr <> '' do
  begin
    s := copy(TempStr, 1, pos(#13#10, TempStr) - 1);
    delete(TempStr, 1, pos(#13#10, TempStr) + 1);
    Site := copy(s, 1, pos(Delimitador, s) - 1) + delimitador;
    delete(s, 1, pos(delimitador, s) - 1);
    delete(s, 1, length(delimitador));
    while s <> '' do
    begin
      Name := copy(s, 1, pos(delimitador, s) - 1) + delimitador;
      delete(s, 1, pos(delimitador, s) - 1);
      delete(s, 1, length(delimitador));
      Value := copy(s, 1, pos(delimitador, s) - 1) + delimitador;
      delete(s, 1, pos(delimitador, s) - 1);
      delete(s, 1, length(delimitador));
