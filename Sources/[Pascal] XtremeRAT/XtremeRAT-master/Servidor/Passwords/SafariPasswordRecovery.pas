...
begin
  ...
  begin
    ...
    while pos(Delimitador, s) > 0 do
    begin
      Result := Result + Copy(s, 1, pos(Delimitador, s) - 1) + Delimitador;
      Delete(s, 1, pos(Delimitador, s) - 1);
  ...
  begin
    site := 'http://' + Copy(Result, 1, pos('(', Result) - 1);
    delete(result, 1, pos('(', Result));

    User := Copy(Result, 1, pos(Delimitador, Result) - 2);
    delete(result, 1, pos(Delimitador, Result) - 1);
    delete(result, 1, Length(Delimitador));

    Pass := Copy(Result, 1, pos(Delimitador, Result) - 1);
    delete(result, 1, pos(Delimitador, Result) - 1);
    delete(result, 1, Length(Delimitador));
