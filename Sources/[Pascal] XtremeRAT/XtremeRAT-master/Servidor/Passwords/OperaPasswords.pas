...
begin
  ...
  if pos('Log profile', Resultado) > 0 then
  begin
    delete(resultado, 1, pos('Log profile', Resultado) + 12);

    while (pos('ftp://', resultado) <> 1) and
          (pos('*ftp://', resultado) <> 1) and
          (pos('http://', resultado) <> 1) and
          (pos('https://', resultado) <> 1) do
    if resultado = '' then break else delete(resultado, 1, 1);

    while resultado <> '' do
    begin
      site := Copy(resultado, 1, pos(#13#10, resultado) - 1);
      if pos('ftp://', site) > 0 then
      begin
        delete(resultado, 1, pos(#13#10, Resultado) + 1);
        login := Copy(resultado, 1, pos(#13#10, resultado) - 1);
        delete(resultado, 1, pos(#13#10, Resultado) + 1);
        pass := Copy(resultado, 1, pos(#13#10, resultado) - 1);
        delete(resultado, 1, pos(#13#10, Resultado) + 1);
      ...
      begin
        ...
        if (pos('ftp://', resultado) = 1) or
           (pos('*ftp://', resultado) = 1) or
           (pos('http://', resultado) = 1) or
           (pos('https://', resultado) = 1) then
        begin
          delete(resultado, 1, pos(#13#10, Resultado) + 1);
          delete(resultado, 1, pos(#13#10, Resultado) + 1);
        end else delete(resultado, 1, pos(#13#10, Resultado) + 1);
        ...
        until (pos('ftp://', resultado) = 1) or
              (pos('http://', resultado) = 1) or
              (pos('https://', resultado) = 1) or
              (Length(resultado) = 0);
      end;
    end;
  end;
