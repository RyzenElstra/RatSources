...
begin
  ...
  begin
    TempFile := Copy(FileList, 1, pos(delimitadorComandos, FileList) - 1);
    delete(FileList, 1, pos(delimitadorComandos, FileList) - 1);
...
begin
  ...
  try
    repeat
      ...
      if InSearch = False then if pos('*' + LowerString(ExtractFileExt(AnsiString(Find.cFileName))), Filter) > 0 then
      begin
        if StopDownloadAll = True then Break;
        Continue;
      end;

      if InSearch = True then if pos('*' + LowerString(ExtractFileExt(AnsiString(Find.cFileName))), Filter) <= 0 then
      begin
        if StopDownloadAll = True then Break;
        Continue;
      end;
