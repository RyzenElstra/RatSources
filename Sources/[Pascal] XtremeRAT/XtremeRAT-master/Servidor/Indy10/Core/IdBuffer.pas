...
}
{
  ...
  Merged the buffer Classes, StreamUnit.
...

{
  ...
  overhead of requiring class creation. These Classes, StreamUnit can be used to copy for .net
  ...
  directly into the byte array instead of the previous Classes, StreamUnit peformance should
  be fine.

  In short - copy under VCL should be acceptable if we watch for bottlenecks and
  fix them appropriately without having to creat proxy Classes, StreamUnit. The only problem
