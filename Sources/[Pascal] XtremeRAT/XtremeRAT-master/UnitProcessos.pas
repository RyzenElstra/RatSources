...
begin
  ...
  begin
    ...
    begin
      try
        Date1 := StrToDateTime(Copy(Item1.SubItems[5], 1, pos(' ', Item1.SubItems[5]) - 1));
        except
      end;
      try
        Date2 := StrToDateTime(Copy(Item2.SubItems[5], 1, pos(' ', Item2.SubItems[5]) - 1));
        except
      end;
...
begin
  ...
  begin
    ...
    try
      ...
      TempInt := StrToInt64(Copy(Recebido, 1, pos(DelimitadorComandos, Recebido) - 1));
      ...
      begin
        TempStr := Copy(Recebido, 1, pos(#13#10, Recebido));
  ...
  begin
    ...
    begin
      AdvListView1.Items.Item[i].SubItems[6] := Copy(Recebido, 1, pos('|', Recebido) - 1);
      break;
    end;
  end else
...
begin
  ...
  AbrirLocalDoArquivo1.Enabled := (AdvListView1.SelCount = 1) and
                                  ...
                                  (pos(':\', AdvListView1.Selected.SubItems[1]) > 0);
