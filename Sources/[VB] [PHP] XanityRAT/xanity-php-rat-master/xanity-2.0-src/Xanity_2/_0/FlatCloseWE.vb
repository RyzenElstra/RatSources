...
Namespace Xanity_2._0
    Friend Class FlatCloseWE
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseWE.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseWE.__ENCList
            SyncLock list
                If (FlatCloseWE.__ENCList.Count = FlatCloseWE.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseWE.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseWE.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseWE.__ENCList.Item(index) = FlatCloseWE.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseWE.__ENCList.RemoveRange(index, (FlatCloseWE.__ENCList.Count - index))
                    FlatCloseWE.__ENCList.Capacity = FlatCloseWE.__ENCList.Count
                End If
                FlatCloseWE.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.Webcam.Close
        End Sub
