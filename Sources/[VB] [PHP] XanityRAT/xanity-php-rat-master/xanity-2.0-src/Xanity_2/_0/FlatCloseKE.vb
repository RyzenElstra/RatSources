...
Namespace Xanity_2._0
    Friend Class FlatCloseKE
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseKE.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseKE.__ENCList
            SyncLock list
                If (FlatCloseKE.__ENCList.Count = FlatCloseKE.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseKE.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseKE.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseKE.__ENCList.Item(index) = FlatCloseKE.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseKE.__ENCList.RemoveRange(index, (FlatCloseKE.__ENCList.Count - index))
                    FlatCloseKE.__ENCList.Capacity = FlatCloseKE.__ENCList.Count
                End If
                FlatCloseKE.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.keylogger.Close
        End Sub
