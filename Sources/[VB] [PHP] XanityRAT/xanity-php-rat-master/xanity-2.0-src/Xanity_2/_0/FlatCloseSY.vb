...
Namespace Xanity_2._0
    Friend Class FlatCloseSY
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseSY.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseSY.__ENCList
            SyncLock list
                If (FlatCloseSY.__ENCList.Count = FlatCloseSY.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseSY.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseSY.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseSY.__ENCList.Item(index) = FlatCloseSY.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseSY.__ENCList.RemoveRange(index, (FlatCloseSY.__ENCList.Count - index))
                    FlatCloseSY.__ENCList.Capacity = FlatCloseSY.__ENCList.Count
                End If
                FlatCloseSY.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.SystemInformation.Close
        End Sub
