...
Namespace Xanity_2._0
    Friend Class FlatCloseSI
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseSI.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseSI.__ENCList
            SyncLock list
                If (FlatCloseSI.__ENCList.Count = FlatCloseSI.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseSI.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseSI.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseSI.__ENCList.Item(index) = FlatCloseSI.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseSI.__ENCList.RemoveRange(index, (FlatCloseSI.__ENCList.Count - index))
                    FlatCloseSI.__ENCList.Capacity = FlatCloseSI.__ENCList.Count
                End If
                FlatCloseSI.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub
