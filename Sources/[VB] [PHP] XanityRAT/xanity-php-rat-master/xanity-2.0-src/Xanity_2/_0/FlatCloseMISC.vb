...
Namespace Xanity_2._0
    Friend Class FlatCloseMISC
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseMISC.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseMISC.__ENCList
            SyncLock list
                If (FlatCloseMISC.__ENCList.Count = FlatCloseMISC.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseMISC.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseMISC.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseMISC.__ENCList.Item(index) = FlatCloseMISC.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseMISC.__ENCList.RemoveRange(index, (FlatCloseMISC.__ENCList.Count - index))
                    FlatCloseMISC.__ENCList.Capacity = FlatCloseMISC.__ENCList.Count
                End If
                FlatCloseMISC.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.misc.Close
        End Sub
