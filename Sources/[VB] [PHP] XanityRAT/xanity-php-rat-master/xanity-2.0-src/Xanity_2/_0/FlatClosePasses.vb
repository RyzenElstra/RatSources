...
Namespace Xanity_2._0
    Friend Class FlatClosePasses
        Inherits Control
        ' Methods
        Public Sub New()
            FlatClosePasses.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatClosePasses.__ENCList
            SyncLock list
                If (FlatClosePasses.__ENCList.Count = FlatClosePasses.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatClosePasses.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatClosePasses.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatClosePasses.__ENCList.Item(index) = FlatClosePasses.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatClosePasses.__ENCList.RemoveRange(index, (FlatClosePasses.__ENCList.Count - index))
                    FlatClosePasses.__ENCList.Capacity = FlatClosePasses.__ENCList.Count
                End If
                FlatClosePasses.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.passes.Close
        End Sub
