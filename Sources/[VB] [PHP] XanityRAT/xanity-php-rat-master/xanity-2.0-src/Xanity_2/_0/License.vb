...
Namespace Xanity_2._0
    Friend NotInheritable Class License
        ...
        Private Function CheckCore() As Boolean
            ...
            If flag2 = (Integer.Parse(ch.ToString) >= 4) Then
                Dim cid As New Guid("9280188D-0E8E-4867-B30C-7FA83884E8DE")
                Dim guid2 As New Guid("B79B0ACD-F5CD-409B-B5A5-A16244610B92")
                Dim meta As IMeta = DirectCast(License.CLRCreateInstance(cid, GetType(IMeta).GUID), IMeta)
            ...
            End If
            Dim path As String = Path.ChangeExtension(Path.Combine(runtimeDirectory, "mscorlib"), "dll")
            ...
            If Not Me.IsTrusted(path, token, flag2) Then
                Return False
            End If
            If Not Me.IsTrusted(str4, token, flag2) Then
                Return False
            End If
            Return True
        End Function

        <DllImport("mscoree.dll", PreserveSig:=False)> _
        Private Shared Function CLRCreateInstance(<MarshalAs(UnmanagedType.LPStruct)> ByVal cid As Guid, <MarshalAs(UnmanagedType.LPStruct)> ByVal iid As Guid) As <MarshalAs(UnmanagedType.Interface)> Object
        End Function
        ...
        Private Sub DownloadLicense()
            Dim startInfo As New ProcessStartInfo With { _
                .Arguments = String.Format("""{0}"" ""{1}"" {2} -s", (Me.Checksums(0) & Me.Checksums(3) & ".co"), Me.LicenseLocation, Me.Checksums(3)), _
                .FileName = Me.GizmoLocation, _
        ...
        Private Function IsTrusted(ByVal path As String, ByVal token As Byte(), ByVal hostCLR As Boolean) As Boolean
            ...
            If ((Not publicKeyToken Is Nothing) AndAlso (publicKeyToken.Length = 8)) Then
                Dim index As Integer = 0
                Do While (publicKeyToken(index) = token(index))
                    index += 1
                    If (index > 7) Then
                        Return True
                    End If
        ...
        Private Sub ValidateSignature()
            If (If((Not Me.ValidateCore OrElse Me.CheckCore), 0, 1) <> 0) Then
