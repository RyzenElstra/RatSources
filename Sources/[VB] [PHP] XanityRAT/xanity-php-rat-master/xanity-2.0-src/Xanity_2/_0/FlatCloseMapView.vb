...
Namespace Xanity_2._0
    Friend Class FlatCloseMapView
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseMapView.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseMapView.__ENCList
            SyncLock list
                If (FlatCloseMapView.__ENCList.Count = FlatCloseMapView.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseMapView.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseMapView.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseMapView.__ENCList.Item(index) = FlatCloseMapView.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseMapView.__ENCList.RemoveRange(index, (FlatCloseMapView.__ENCList.Count - index))
                    FlatCloseMapView.__ENCList.Capacity = FlatCloseMapView.__ENCList.Count
                End If
                FlatCloseMapView.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.MapView.Close
        End Sub
