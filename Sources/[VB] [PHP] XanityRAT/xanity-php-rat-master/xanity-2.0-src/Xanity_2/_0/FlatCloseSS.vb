...
Namespace Xanity_2._0
    Friend Class FlatCloseSS
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseSS.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseSS.__ENCList
            SyncLock list
                If (FlatCloseSS.__ENCList.Count = FlatCloseSS.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseSS.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseSS.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseSS.__ENCList.Item(index) = FlatCloseSS.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseSS.__ENCList.RemoveRange(index, (FlatCloseSS.__ENCList.Count - index))
                    FlatCloseSS.__ENCList.Capacity = FlatCloseSS.__ENCList.Count
                End If
                FlatCloseSS.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.StressTester.Close
        End Sub
