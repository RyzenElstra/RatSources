...
Namespace Xanity_2._0
    Friend Class FlatCloseAU
        Inherits Control
        ' Methods
        Public Sub New()
            FlatCloseAU.__ENCAddToList(Me)
        ...
        Private Shared Sub __ENCAddToList(ByVal value As Object)
            Dim list As List(Of WeakReference) = FlatCloseAU.__ENCList
            SyncLock list
                If (FlatCloseAU.__ENCList.Count = FlatCloseAU.__ENCList.Capacity) Then
                    Dim index As Integer = 0
                    Dim num3 As Integer = (FlatCloseAU.__ENCList.Count - 1)
                    Dim i As Integer = 0
                    Do While (i <= num3)
                        Dim reference As WeakReference = FlatCloseAU.__ENCList.Item(i)
                        If reference.IsAlive Then
                            If (i <> index) Then
                                FlatCloseAU.__ENCList.Item(index) = FlatCloseAU.__ENCList.Item(i)
                            End If
                            index += 1
                        End If
                        i += 1
                    Loop
                    FlatCloseAU.__ENCList.RemoveRange(index, (FlatCloseAU.__ENCList.Count - index))
                    FlatCloseAU.__ENCList.Capacity = FlatCloseAU.__ENCList.Count
                End If
                FlatCloseAU.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
            End SyncLock
        End Sub

        Protected Overrides Sub OnClick(ByVal e As EventArgs)
            MyBase.OnClick(e)
            MyProject.Forms.Audio.Close
        End Sub
