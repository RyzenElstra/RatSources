...
uses
  Windows, SysUtils, UnitFunctions;
...
var
  TmpStr : AnsiString;
begin
  TmpStr := AnsiString(source);
  Result := Crc32(0, PAnsiChar(TmpStr), Length(TmpStr));
end;
//-----
...
begin
  TmpStr := '';
  if GetCPUSerialNumber <> '' then TmpStr := TmpStr + GetCPUSerialNumber;
  if Trim(string(GetIdeSerialNumber)) <> '' then TmpStr := TmpStr + Trim(string(GetIdeSerialNumber));
  if TmpStr <> '' then NumberId := CrcString(TmpStr);
  Result := GetComputer + '^(' + IntToStr(NumberId) + ')';
end;
