unit UnitActiveConnections;

interface

uses
  Windows, WinSock, TlHelp32, PsAPI, UnitFunctions;
...
const
  TCP_STATES: array[1..12] of string = (ResTCPStateClosed, ResTCPStateListen,
                                        ResTCPStateSynSent, ResTCPStateSynRcvd,
                                        ResTCPStateEst, ResTCPStateFW1,
                                        ResTCPStateFW2, ResTCPStateCloseWait,
                                        ResTCPStateClosing, ResTCPStateLastAck,
                                        ResTCPStateTimeWait, ResTCPStateDeleteTCB);
...
function ListActiveConnections(ResolveDNS: boolean): string;
function CloseActiveConnection(LocalHost, RemoteHost: string; LocalPort, RemotePort: integer): boolean;
...
begin
  ...
  Result := IntToStr(bytes[0]) + '.' + IntToStr(bytes[1]) + '.' + IntToStr(bytes[2]) + '.' + IntToStr(bytes[3]);
end;
...
begin
  ...
  begin
    ...
    if ( (GetIP(item.dwLocalAddr) = LocalHost) or (GetNameFromIP(GetIP(item.dwLocalAddr)) = LocalHost) ) and
      ...
      (GetPort(item.dwRemotePort) = RemotePort)
    then
    begin
...
var
  ...
  dwSize: DWORD;
begin
  if not IPHelperLoaded then Exit;
  WSAStartup(2, wsadata);
  try
    if IPHelperVistaLoaded then
    begin
      dwSize := 0;
    ...
    else if IPHelperXPLoaded then AllocateAndGetTcpExTableFromStack(@FBufferTCP, True, GetProcessHeap, 2, 2);
  finally
    WSACleanup;
  end;
end;
...
begin
  ...
  try
    ...
    else if IPHelperXPLoaded then AllocateAndGetTcpExTableFromStack(@FBufferUdp, True, GetProcessHeap, 2, 2);
  finally
    WSACleanup;
  end;
end;
...
begin
  ...
  IPHelperXPLoaded := IPHelperLoaded and (Assigned(AllocateAndGetTcpExTableFromStack) and Assigned(AllocateAndGetUdpExTableFromStack));
  IPHelperVistaLoaded := IPHelperLoaded and (Assigned(GetExtendedTcpTable) and Assigned(GetExtendedUdpTable));
end;
...
begin
  ...
  repeat if Entry.th32ProcessID = PID then Result := Entry.szExeFile; until
    not Process32Next(ProcessHandle, Entry);
end;
...
begin
  ...
  begin
    ...
    if ResolveDNS = false then LocalHost := GetIP(item.dwLocalAddr) else
      LocalHost := GetNameFromIP(GetIP(item.dwLocalAddr));
    LocalPort := GetPort(item.dwLocalPort);
    if ResolveDNS = false then RemoteHost := GetIP(item.dwRemoteAddr) else
    ...
    Result := Result + inttostr(PID) + '|' + procname + '|' + Protocol + '|' +
      LocalHost + ':' + inttostr(LocalPort) + '|' + RemoteHost + ':' + inttostr(RemotePort) + '|' +
      status + '|' + ProcessFileName(PID) + '|' + #13#10;
  end;
  ...
  begin
    ...
    Protocol := 'UDP';
    if ResolveDNS = false then LocalHost := GetIP(item2.dwLocalAddr) else
      LocalHost := GetNameFromIP(GetIP(item2.dwLocalAddr));
    ...
    Result := Result + inttostr(PID) + '|' + procname + '|' + Protocol + '|' +
      LocalHost + ':' + inttostr(LocalPort) + '|' + RemoteHostUDP + ':' + RemotePortUDP + '|' +
      status + '|' + ProcessFileName(PID) + '|' + #13#10;
  end;
end;
